...
## 2026-03-25 - AI 미션 도우미 (AIMissionHelper) 이식
WHY: 워크북 페이지에서 미션 진행 중 막힐 때 AI에게 바로 질문할 수 있는 도우미 필요
HOW: 실험마켓(tryout-market-front) AIMissionHelper 패턴을 camp 프로젝트에 맞게 이식

산출물:
- `/src/components/AIMissionHelper.tsx` — 문어쌤 AI 채팅 컴포넌트
  - Desktop: 우측 사이드바 패널 (380px), Mobile: FAB + 풀스크린
  - Quick chips: '오늘 뭐 해야 해?', '어디서 막혔는지 모르겠어', '내가 쓴 거 봐줘', '예시 보여줘'
  - ReactMarkdown + remark-gfm으로 AI 응답 렌더링
  - 오렌지 포인트 + 화이트 배경 디자인
  - 로딩 애니메이션 (bounce dots)
- `/src/app/api/chat/workbook/route.ts` — 워크북 전용 챗봇 API
  - claude-haiku-4-20250414 모델 (비용 절약)
  - 문어쌤 캐릭터 시스템 프롬프트 (따뜻하지만 현실적, 2-4문장 답변)
  - Day/미션 컨텍스트 포함
- DayWorkbook.tsx에 통합 (desktopOpen 상태 + 미션 컨텍스트 전달)
- 의존성 추가: react-markdown, remark-gfm (pnpm)
- 기존 /api/chat (신청폼 챗봇) 미변경
✅ 빌드 성공

---

## 2026-03-26 - 워크북 디자인을 실험마켓 디자인 시스템으로 전면 교체
WHY: camp 스타일(검정+형광노란)에서 실험마켓 스타일(밝은 배경+오렌지 포인트)로 통일
HOW: DayWorkbook.tsx 전면 재작성 + globals.css, layout.tsx, content.ts 수정

변경 내용:
- **globals.css**: body 폰트를 Pretendard Variable로 변경
- **layout.tsx**: Pretendard CDN 웹폰트 `<link>` 추가
- **content.ts**: ContentBlock 타입에 `input`(빈칸 채우기), `checkbox`(체크리스트) 타입 추가
- **DayWorkbook.tsx**: 전면 재디자인
  - bg-black → bg-[#f8fafc] 밝은 배경
  - #c8ff00 형광노란 → orange-500 포인트 컬러
  - Step 카드: 흰색 bg + 접이식(collapsible) + 완료 체크(localStorage)
  - 프롬프트 블록: bg-gray-50, 팁 블록: bg-orange-50 + border-l-4
  - 미션 완료 박스: bg-orange-50 border-orange-200
  - 헤더: sticky bg-white/80 backdrop-blur + 오렌지 프로그레스 바
  - InputField / CheckboxItem 컴포넌트 추가 (localStorage 저장)
- **AIMissionHelper.tsx**: 이미 오렌지 톤이므로 변경 불필요
✅ 빌드 성공
